...
const Contact = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await emailjs.send(
        ...
        {
          name: formData.name,            // {{name}} in EmailJS
          email: formData.email,          // {{email}} in EmailJS (and used for Reply To)
          message: formData.message,      // {{message}} in EmailJS
          from_name: formData.name,       // {{from_name}} (used in Subject line if needed)
          time: new Date().toLocaleString() // {{time}} in EmailJS
        },
  ...
  return (
    <section id="contact" className="pt-40 pb-20 bg-gray-900">
